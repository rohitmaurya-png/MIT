    ...
    for(var i = 0; i < array.length; i++) {
        Event.findOne({linkName: elink}, function (err, event) {
            ...
            //non team event
                ...
                Account.find({mit_id: {$in: array}}).lean().exec(function(err, users) {
                    var ids = [];
                    for (var i in users) {
        ...
        if (!event || err) {
            ...
            Account.find({mit_id:{ $in: mit }},function(err, users) {
                ...
                if (err) {
                    ...
                    res.redirect('/events/' + event.linkName + '/participants/invalid-mit-ids');
                }
            });
        }
    });
    ...
    Event.findOne({linkName: req.params.eventLink},
        function (err, event) {
            if(!event || err ) {
                ...
                if (!event.isTeamEvent) {
                    Account.find({_id: {$in: list}}).lean().exec(function (err, users) {
                        res.xls(event.linkName + '.xlsx', users, {fields: ['mit_id', 'firstName', 'lastName', 'email', 'phone_no', 'college', 'course']});
                    })
                } else {
                    Team.find({_id: {$in: list}}).populate('members captain').lean().exec(function(err, teams) {
                        var out = [];
                        ...
                        res.xls(event.linkName + '.xlsx', out, {fields: ['mit_id', 'firstName', 'lastName', 'email', 'phone_no', 'college', 'course']});
                    })
                }
            }
        })
